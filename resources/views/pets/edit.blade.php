                    ...
                    <form action="{{ route('pets.update', $pet['id']) }}" method="post">
                        ...
                        <div class="mb-3 row">
                            ...
                            <select class="form-select" name='category'>
                                ...
                                @foreach ($categories as $c)
                                    <option {{ $pet['category']['id'] === $c['id'] ? 'selected' : '' }}
                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-md-6">
                                <input class="form-control @error('photoUrls') is-invalid @enderror" id="photoUrls"
                                    name="photoUrls" value="{{ implode(',', $pet['photoUrls']) }}">
                        ...
                        <div class="mb-3 row">
                            ...
                            <select multiple class="form-select" name='tags[]' id='tags'>
                                ...
                                @foreach ($tags as $t)
                                    <option {{ $pet['tags'] && (false !== array_search((string) $t['id'], $pet['tags'])) ? 'selected' : '' }}
                                        value="{{ $t['id'] }}">
                                        {{ $t['name'] }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3 row">
                            ...
                            <select class="form-select" name='status'>
                                ...
                                @foreach ($statuses as $s)
                                    <option {{ pet['status'] === $s ? 'selected' : '' }} value="{{ $s }}">
                                        {{ $s }}</option>
                                @endforeach
                            </select>
                        </div>
